...
import { COUNTRY_CONSTANT, COUNTRY_PAGE_CONSTANT } from "../../components/CountryConstants";
import './country-page.scss'

const destinationList = CommonUtils.getKeyValuesFromArrayUnderCaps(COUNTRY_PAGE_CONSTANT,"DESTINATION");
const CountryPage = () => {
    ...
    <>
        ...
        <Row>
            <Col className="flex-center" xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                <h1 className="stylish-font destination-header ">{fetchCountry(params.countryName).at(0).toUpperCase() + fetchCountry(params.countryName).slice(1).toLowerCase().replaceAll("-"," ")}</h1>
            </Col>
        </Row>
            <Row>
                <Col className="" xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                    <div className="destination-text destination-font">
                    { CommonUtils.searchKeyValuesFromArrayUnderCaps(COUNTRY_PAGE_CONSTANT,"DESTINATION",fetchCountry(params.countryName)).DESCRIPTION ? CommonUtils.searchKeyValuesFromArrayUnderCaps(COUNTRY_PAGE_CONSTANT,"DESTINATION",fetchCountry(params.countryName)).DESCRIPTION.split("\n").map(item => (
